...
{   
	uint16_t retry = 0;
	while(!LL_SPI_IsActiveFlag_TXE(SPI2)){	//	检查发送缓冲区是否为空
		if(++retry > 200)
			return 0;
	}

	LL_SPI_TransmitData8(SPI2, TxData);		//	发送数据
	retry = 0;
	
	while(!LL_SPI_IsActiveFlag_RXNE(SPI2)){	//	检查接收缓冲区非空
		if(++retry > 200)
			return 0;
	}
	
 	return LL_SPI_ReceiveData8(SPI2);    //返回收到的数据	
}
...
uint16_t SPI_ByteRead(unsigned char addr)                                                
{
	...
	while((SPI2->SR & 1<<1) == 0);		//wait for tx buf empty
	SPI2->DR = addr; 
