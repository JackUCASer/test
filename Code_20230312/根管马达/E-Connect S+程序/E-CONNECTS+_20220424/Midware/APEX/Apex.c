...
SAM_DAT samh,saml;
Str_FL flag;										// 高低频采样切换标志位
uint32_t SamH[20];									// 高频信号采样数据缓存
uint32_t SamL[20];									// 低频信号采样数据缓存
uint32_t Ratio_mul;									
QUE_def samH_Que,samL_Que;							// 滤波器缓存队列

uint32_t	CAL_APEX_CAL_3K6_100nF;					// 开机时内部根测校准，实测值			
uint32_t 	samH_RootInPosTemp = 480;				
uint32_t  	samH_RootInPos = 550;					// 判断进入根管的标记 根据高频信号
uint32_t  	samH_RootOutPos = 630;					// 判断离开根管的标记
uint8_t 	InRoot = 0;
uint8_t 	RunWithApex_InRootFlag = 0; 			// 边测边扩功能进入根管标志位
uint8_t 	Last_apex_pos = 24;						// 根测显示的位置控制，上一个位置点记录
unsigned int Ratio;

const unsigned int Init_APEX_3K6_100nF =755;		// 出厂时的定值，不可更改的
float E_rate;       								// 误差变动率
...
void Apex_cal()
{
    ...
    HAL_Delay(150);
	flag_cal = 1;																// 算法选择  =1，加入滤波算法；=0，简单算法
    while(i < 10) 
	{
		if(flag.sam_finish == 1) 												// 数据测量完成，进入计算
		{
			flag.sam_finish = 0;
	...
	flag_cal = 0;																// 算法选择  =1，加入滤波算法；=0，简单算法
...
{	
	...
	if(flag.samL_Rise_begin == 1)
	{
		...
		if(i >= 6)
		{
			...
			samh.sum = 0;
			saml.sum = 0;
			for(j = 1; j < 6; j++) 									//抛弃第一个数
			{
				samh.sum += SamH[j];
				saml.sum += SamL[j];
			}
			samh.min = samh.sum / 5;								//获取高频谷值，低频峰值
			saml.max = saml.sum / 5;
	...
	if(flag.samH_Rise_begin == 1)
	{
		...
		if(k>=6)
		{
			...
			samh.max = samh.sum / 5;      							//获取高频峰值，低频谷值
			saml.min = saml.sum / 5;
           //---采样偏差控制--------高频峰值------------
...
/*****************************原算法**********************************/
	...
	if((flag.sam_finish == 1)&&(flag_cal == 0)&&(flag.sam_apex_begin == 0))
	{
		/****  未进入根管，检测信号是否满足********/
...
//		flag.sam_finish = 0;
		flag.sam_apex_begin = 1;
	}
	  static unsigned int Last_Apex_num = 0;
		

    if((flag.sam_apex_begin == 1)&&(flag_cal == 0)) 
	{	
