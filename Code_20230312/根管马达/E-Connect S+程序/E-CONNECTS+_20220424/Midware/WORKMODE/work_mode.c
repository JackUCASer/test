...
WorkMode_e Rotory(WorkMode_e mode)
{
	...
	//故障检测
	if((status_mp6570 != 0x04)&&(flag_running == 1))														//检测到MP6570由于故障不处于工作状态
	{
		stop();
		delay_ms(10);
		LOOPTIMER_DISABLE;
...
//		Disp_torque_bar_scale1(Disp_Torque+5,TL_Motor_Last,Disp_TL_limit_flag);
//		TL_Motor_Last = Disp_Torque+5;
		Disp_TL_cnt = 0;
		Read_ig_flag = 0;
	}
...
WorkMode_e Reciproc(WorkMode_e mode)
{
	if((flag_running == 0) && (LVGL_TL_Flag == 1))
	{
		...
		{	
			if(sel.AngleCW < 16)
			{
				motor_settings.upper_threshold = AngleCW_T[sel.AngleCW]*d_Torque[sel.Torque]/10 + (16 - sel.AngleCW)*sel.RecSpeed;			//转矩保护上限设置
			}else
			{
		...
		}else
		{
			if(sel.AngleCCW < 16)
			{
				motor_settings.upper_threshold = AngleCW_T[sel.AngleCCW]*d_Torque[sel.Torque]/10 + (16 - sel.AngleCCW)*sel.RecSpeed;	//转矩保护上限设置
			}else
			{
...
//		LOOPTIMER_ENABLE;
		...
		{	
			if(sel.AngleCW < 16)
			{
				motor_settings.upper_threshold = (AngleCW_T[sel.AngleCW]*d_Torque[sel.Torque]/10 + (16 - sel.AngleCW)*sel.RecSpeed)*4/3- NoLoadCurrent[r_d_speed[sel.RecSpeed]+28]- NoLoadCurrent[r_d_speed[sel.RecSpeed]];			//转矩保护上限设置
			}else
			{
				motor_settings.upper_threshold = (AngleCW_T[sel.AngleCW]*d_Torque[sel.Torque]/10)*4/3- NoLoadCurrent[r_d_speed[sel.RecSpeed]+28]- NoLoadCurrent[r_d_speed[sel.RecSpeed]];			//转矩保护上限设置
			}
			motor_settings.lower_threshold = motor_settings.upper_threshold*15/16  - NoLoadCurrent[r_d_speed[sel.RecSpeed]+28]- NoLoadCurrent[r_d_speed[sel.RecSpeed]];			//转矩取消保护下线设置
		}else
		{
			if(sel.AngleCCW < 16)
			{
