...
void Disp_cal(void)
{
	...
	lv_style_set_bg_color(&style_Cal_theme, lv_palette_lighten(LV_PALETTE_GREY,3));											//设置上背景颜色
	lv_style_set_border_width(&style_Cal_theme, 0);																			//设置边框宽度
	lv_style_set_text_color(&style_Cal_theme, lv_color_black());															//设置样式中文字颜色		
...
void Standy_sel_init(FUN_SEL sel)
{
	...
	lv_style_set_bg_color(&style_sel_theme, lv_palette_lighten(LV_PALETTE_GREY,3));											//设置上背景颜色
	lv_style_set_border_width(&style_sel_theme, 0);																			//设置边框宽度
	lv_style_set_text_color(&style_sel_theme, lv_color_black());															//设置样式中文字颜色		
...
//		static lv_style_t 	Disp_torque_bar_style;
//		lv_style_reset(&Disp_torque_bar_style);
//		lv_style_init(&Disp_torque_bar_style);
//		lv_style_set_radius(&Disp_torque_bar_style, 0);															//设置圆角0的边框
//		lv_style_set_border_opa(&Disp_torque_bar_style, LV_OPA_0);												//设置边框透明
//		lv_style_set_bg_color(&Disp_torque_bar_style, lv_palette_lighten(LV_PALETTE_RED,3));					//设置背景色为LV_COLOR_WHITE
//		lv_style_set_bg_grad_color(&Disp_torque_bar_style, lv_palette_main(LV_PALETTE_RED));
//		lv_style_set_bg_grad_dir(&Disp_torque_bar_style, LV_GRAD_DIR_HOR);										//从左到右渐变
//		lv_style_set_width(&Disp_torque_bar_style, TL_Motor);													//设置样式宽度
//		lv_style_set_height(&Disp_torque_bar_style, 25);														//设置样式高度
//		
//		TL_motor_bar = lv_obj_create(lv_scr_act());																//创建目标   lv_scr_act()接口来获取当前活跃的屏幕对象
//		lv_obj_add_style(TL_motor_bar , &Disp_torque_bar_style, 0);												//加载一种样式
//		lv_obj_set_scrollbar_mode(TL_motor_bar ,LV_SCROLLBAR_MODE_OFF);											//不显示滚动条
//		lv_obj_align(TL_motor_bar , LV_ALIGN_BOTTOM_LEFT, 0, -23);												//设置目标位置
...
void SYS_set_init(FUN_SET set)
{
	...
	lv_style_set_bg_color(&style_set_theme, lv_palette_lighten(LV_PALETTE_GREY,3));											//设置上背景颜色
	lv_style_set_border_width(&style_set_theme, 0);																			//设置边框宽度
	lv_style_set_text_color(&style_set_theme, lv_color_black());															//设置样式中文字颜色		
...
void Seq_list_init(Seq_parameter_inside *fSeq_Inside, int name_str, int Seq_Disp_str, int Seq_parameter_str)
{
	...
	lv_style_set_bg_color(&style_list_theme, lv_palette_lighten(LV_PALETTE_GREY,3));										//设置上背景颜色
	lv_style_set_border_width(&style_list_theme, 0);																		//设置边框宽度
	lv_style_set_text_color(&style_list_theme, lv_color_black());															//设置样式中文字颜色		
...
void New_File_init(int add_file_number, Seq_parameter_outside fNew_file_parameter, int Seq_const_len ,int Seq_str)
{
	...
	lv_style_set_bg_color(&style_new_theme, lv_palette_lighten(LV_PALETTE_GREY,3));											//设置上背景颜色
	lv_style_set_border_width(&style_new_theme, 0);																			//设置边框宽度
	lv_style_set_text_color(&style_new_theme, lv_color_black());															//设置样式中文字颜色		
