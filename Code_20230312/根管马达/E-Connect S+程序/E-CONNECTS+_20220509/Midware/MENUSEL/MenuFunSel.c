...
void DispMenuCWAngleUp(void)
{
	...
	if((sel.AngleCCW + sel.AngleCW) < 6)				//限制最小等角度
	{				
		sel.AngleCW = 6 - sel.AngleCCW;
	}
...
{
	sel.AngleCW--;
	if(sel.AngleCW<0 || (sel.AngleCCW + sel.AngleCW) < 6)
	{
...
void DispMenuCCWAngleUp(void)
{
	...
	if((sel.AngleCW + sel.AngleCCW) < 6)					//限制最小等角度
	{				
		sel.AngleCCW = 6 - sel.AngleCW;
	}
	Standy_sel_init(sel);
};
...
{
	sel.AngleCCW--;
	if((sel.AngleCCW < 0) || (sel.AngleCW + sel.AngleCCW) < 6)
	{
