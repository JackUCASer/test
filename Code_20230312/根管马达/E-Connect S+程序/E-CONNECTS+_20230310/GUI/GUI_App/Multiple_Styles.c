...
void Seq_Outsidelist_init(Seq_parameter_outside fNew_file_parameter, int name_str, int Seq_Disp_str, int Seq_parameter_str)
{
	...
	if(Seq_parameter_str >= fNew_file_parameter.seq_Len)
	{
		...
		if(set.Language == EN)
		{
			lv_label_set_text(data1_label, "Add one");					//设置动态文本(字符串形式)
		}else
		{
			lv_label_set_text(data1_label, CN_XIN CN_ZENG );					//设置动态文本(字符串形式)
		}
...
//	lv_obj_align(warning_img, LV_ALIGN_LEFT_MID, -5, -15);
	...
	lv_obj_align(delete_label, LV_ALIGN_TOP_MID, 0, 0);
	lv_obj_align(delete_label1, LV_ALIGN_BOTTOM_MID, 0, 5);
}

/**********************************************************************************************************
*	函 数 名: void lv_Calibrating_init(void)
*	功能说明: 校准中
*	形    参: int Battary_bar 电量
*	返 回 值: 无
*	编 辑 者: 1：王昌盛
...
*	编辑日期: 1: 20211031         
**********************************************************************************************************/
void lv_RestoreFactory_init(void)
{
	lv_obj_del(sel_top);		
	sel_top = lv_obj_create(NULL);																							//为显示器创建一个 lv_obj 类型的基础对象
	lv_scr_load(sel_top);
	...
	lv_obj_t* textFac=lv_label_create(sel_top);
	if(set.Language==EN){
		lv_label_set_text(textFac,"Recovering...");
	}
	else if(set.Language==CN){
		lv_label_set_text(textFac,CN_ZHENG CN_ZAI1 CN_HUI2 CN_FU CN_CHU1 CN_CHANG CN_SHE CN_ZHI);
	}
	lv_obj_align(textFac,LV_ALIGN_CENTER,0,0);				//设置文字位置	
}
/**********************************************************************************************************
