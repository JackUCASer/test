...
void Battery_init(void)
{
	...
	do
	{
		...
		//电量检测
		if(BatVol_mv < 3300)														//电量划分，为显示做准备
		{
			sys.BatteryLevel = 0;
		}
		else if((BatVol_mv >= 3300) && (BatVol_mv < 3500))
		{                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            
			sys.BatteryLevel = 1;
		}
		else if((BatVol_mv >= 3500) && (BatVol_mv < 3700))
		{
			sys.BatteryLevel = 2;
		}
		else if((BatVol_mv >= 3700) && (BatVol_mv < 3900))
		{
			sys.BatteryLevel = 3;
		}
		if(BatVol_mv > 3900)
...
void SamBattery(void)
{
	...
	if(Battery_COUNT_IT >= 10)
	{
		...
		if((Disp_bat_cnt > 200) &&(0 == Get_Index())&&(ADC_ConvertedValue[0] > 100))
		{	//2s 刷新一次
			Disp_bat_cnt = 0;															//计数清零
//			Start_Adc();																//启动ADC检测电池电压
			BatVol_mv = (ADC_ConvertedValue[0] * Ref_v) >> 11;							//计算电池两端电压，mv
			...
			}else if(sys.BatteryLevel == 1)
			{
				if(BatVol_mv >= 3550)
				{
					sys.BatteryLevel = 2;
				}else if(BatVol_mv < 3280)
				{
					sys.BatteryLevel = 0;
				}
			}else if(sys.BatteryLevel == 2)
			{
				if(BatVol_mv >= 3750)
				{
					sys.BatteryLevel = 3;
				}else if(BatVol_mv < 3480)
				{
					sys.BatteryLevel = 1;
				}
			}else if(sys.BatteryLevel == 3)
			{
				if(BatVol_mv >= 3950)
				{
					sys.BatteryLevel = 4;
				}else if(BatVol_mv < 3680)
				{
					sys.BatteryLevel = 2;
				}
			}else if(sys.BatteryLevel == 4)
			{
				if(BatVol_mv < 3880)
				{
					sys.BatteryLevel = 3;
