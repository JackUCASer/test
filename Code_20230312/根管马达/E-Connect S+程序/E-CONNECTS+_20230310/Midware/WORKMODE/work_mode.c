...
int 		iq_motor = 0;
int 		iq_sum = 0;
uint16_t    iq_WUXIAO_cnt = 0;

uint8_t ap_Action_flag = 0;							//在速度模式作用， = 0，未到根测点   = 1，到达根测点
...
uint8_t LVGL_ON_OFF = 1;								// = 1,开启LVGL；= 0，关闭LVGL

extern uint16_t	position_cnt_limit;
extern uint16_t	position_SUM_cnt_limit;
...
extern uint8_t iq_shuaxin_flag;
extern uint8_t motor_qiehuan_flag;
uint16_t  Disp_cnt = 0;
uint16_t motor_Cnt;
/**********************************************************************************************************
...
uint8_t RoDebug_pw = 0;								// 在运行模式下，检测按键（M10模式，速度1000，密码检测用）
uint8_t Aging_Flag = 0;								// 老化模式标志，=1，进入老化模式；=0，进入速度模式
uint32_t Aging_cnt;									// 老化模式下启停电机计数
uint8_t Aging_ON_OFF;								// 老化模式下，=0，电机停止；=1，电机开启
uint8_t Aging_Flag1 = 0;								// 老化模式标志，=1，进入老化模式；=0，进入速度模式
uint32_t Aging_cnt1;									// 老化模式下启停电机计数
uint8_t Aging_ON_OFF1;								// 老化模式下，=0，电机停止；=1，电机开启
uint16_t Torque_Display_cnt;							// =0,1024次显示一下，=1,1024次显示一下，=2,8192显示一次
uint8_t psw[4] = {0};
WorkMode_e Rotory(WorkMode_e mode)
{
	uint8_t i;
	static 	uint8_t Num = 0;
	static 	uint8_t Disp_T_ON_OFF = 0;
	static 	int TL_Motor_Last = 0;
	...
	if((flag_running == 0) && (LVGL_TL_Flag == 1))
	{
		...
		{
			HAL_GPIO_WritePin(VM_CTL_GPIO_Port,VM_CTL_Pin,GPIO_PIN_SET);			//设置电机电压为9.5V
		}
		...
		}else if((sel.Torque>11) &&(sel.Torque <=13))
		{
			Torque_limit = S_T_I_COE4[sel.Torque-12][sel.Speed];
		}else
		{
			Torque_limit = S_T_I_COE4[1][sel.Speed];				//RL固定在5.0
		}
		...
		if(sel.OperaMode == 0)																									//正转
		{
			...
			if(sel.Torque == 14)
			{
				motor_settings.upper_threshold =  500;			//转矩保护上限设置
				motor_settings.lower_threshold =  250;			//转矩取消保护下线设置	
			}else if((sel.Torque == 0)||(sel.Torque == 1))
			{
				motor_settings.upper_threshold =  Torque_limit - NoLoadCurrent[sel.Speed] - NoLoadCurrent[sel.Speed+32];			//转矩保护上限设置
				motor_settings.lower_threshold =  (Torque_limit- NoLoadCurrent[sel.Speed])*3/4 + NoLoadCurrent[sel.Speed];			//转矩取消保护下线设置	
			}else 
			{
				motor_settings.upper_threshold =  Torque_limit - NoLoadCurrent[sel.Speed] - NoLoadCurrent[sel.Speed+32];			//转矩保护上限设置
		...
		else if(sel.OperaMode == 1)																									//反转
		{
			...
			}else if((sel.Torque == 0)||(sel.Torque == 1))
			{
				...
				motor_settings.lower_threshold = (Torque_limit - NoLoadCurrent[sel.Speed])*3/4 - NoLoadCurrent[sel.Speed+32];			//转矩取消保护下线设置	
			}else
			{
				motor_settings.upper_threshold =  Torque_limit;																			//转矩保护上限设置
				motor_settings.lower_threshold = (Torque_limit - NoLoadCurrent[sel.Speed])/2 - NoLoadCurrent[sel.Speed+32];				//转矩取消保护下线设置	
			}		
		}
		if(sel.Torque == 14)
		{
			Torque_p = ((float)(Torque_limit - NoLoadCurrent[sel.Speed])) / d_TqBlinkGrid[sel.Torque-1];
		}else
		{
		...
		Torque_Display_cnt = 0;
		Round_Cnt = 0 ;
		start(); 
		iq_WUXIAO_cnt = 0;
		LVGL_ON_OFF = 0;																										//关闭LVGL
		TL_Motor_Last = 0;
		Disp_T_ON_OFF = 0;
		Disp_cnt = 0;
		iq_shuaxin_flag = 0;
	...
	//故障检测
	if((Motor_Error_flag == 1)&&(flag_running == 1))														//检测到MP6570由于故障不处于工作状态
	{
		stop();
		delay_ms(10);
		LOOPTIMER_DISABLE;
		...
		flag_running = 0;
		Motor_Error_flag = 0;
		Disp_torque_bar_scale1(0,d_TqBlinkGrid[sel.Torque],Disp_TL_limit_flag);
	}else
	{	
		CalculateApex();	
	}
	
	if(ADC_ConvertedValue[0] < 100)													// AD采样错误，需复位
	{
	...
	//均值滤波
	iq_WUXIAO_cnt ++;
//	if(motor_qiehuan_flag == 1)
//	{
//		if((iq_WUXIAO_cnt > 2500)&&(iq_shuaxin_flag == 1))
//		{
//			iq_shuaxin_flag = 0;
//			iq_WUXIAO_cnt = 2502;
//			iq_sum += motor_iq;
//			Iq_cnt++;
//			motor_qiehuan_flag = 0;
//		}
//	}else
	{
		if((iq_WUXIAO_cnt > 500)&&(iq_shuaxin_flag == 1))
		{
			iq_shuaxin_flag = 0;
			iq_WUXIAO_cnt = 502;
			iq_sum += motor_iq;
	...
	if((Iq_cnt >= Iq_num)&&(Torque_Display_cnt < 50))					//分段滤波
	{
		Torque_Display_cnt ++;
		...
		if(sel.OperaMode == 0)
		{
			...
			{		
				iq_motor -= NoLoadCurrent[sel.Speed+32];
				if(iq_motor > -20)
				{
					iq_motor = 0;
				}
		...
		Read_ig_flag = 1;
	}else if((Iq_cnt >= 8*Iq_num)&&(Torque_Display_cnt >= 50))				//分段滤波
	{
		Torque_Display_cnt = 51;
		iq_motor = iq_sum / Iq_cnt;
		Iq_cnt = 0;
		iq_sum = iq_motor * Iq_num * 4;
		Iq_cnt = Iq_num * 4;
		if(sel.OperaMode == 0)
		{
			if(motor_status.status == Status_FORWARD)
...
//				Ring.long_ring = 1;
	...
	if(Read_ig_flag == 1)
	{
		...
		if(Disp_cnt > 6000)
		{
			if(Disp_T_ON_OFF > 20)
			{
				Disp_torque_bar_scale1(Disp_Torque,TL_Motor_Last,Disp_TL_limit_flag);
				TL_Motor_Last = Disp_Torque;
			}else
			{
				Disp_T_ON_OFF++;
			}
	...
	if((RoDebug_pw!=0)&&((sel.Speed == s1500)||(sel.Speed == s400))&&(mem.MemoryNum == 10))
    {
        psw[Num] = RoDebug_pw;
        Num++;     
		RoDebug_pw = 0;		
	...
	if((Num >= 4)&&(sel.Speed == s1500))
	{
		if((psw[0]==2)&& (psw[1]==3)&& (psw[2]==1)&&(psw[3]==1))		//【>】【<】【S】【S】
		{
			Aging_Flag = 1;
			Num = 0;
			Aging_cnt = 0;
			Aging_ON_OFF = 1;						// 由运行模式进入老化模式，一定时先开机状态
			LVGL_ON_OFF = 1;						//关闭LVGL
			Standy_ON(0,1);
		}
		else
		{
	...
	if((Num >= 4)&&(sel.Speed == s400))
	{
		if((psw[0]==2)&& (psw[1]==3)&& (psw[2]==2)&&(psw[3]==3))		//【>】【<】【S】【S】
		{
			Aging_Flag1 = 1;
			Num = 0;
			Aging_cnt = 0;
...
WorkMode_e Reciproc(WorkMode_e mode)
{
	if((flag_running == 0) && (LVGL_TL_Flag == 1))
	{
		...
		motor_settings.toggle_mode_speed = r_Speed[sel.RecSpeed]+100;			  																				//往复模式速度设置
		motor_settings.forward_position = d_AngleCCW[sel.AngleCW]-29;																						//向前转动角度  前转为负，反转为正
		motor_settings.reverse_position = d_AngleCW[sel.AngleCCW]+29;																						//向后转动角度
		
		if( -motor_settings.forward_position >= motor_settings.reverse_position )
		{	
			if(sel.AngleCCW < 16)
			{
				motor_settings.upper_threshold = AngleCW_T[sel.AngleCW]*d_Torque[sel.Torque]/10 + (16 - sel.AngleCCW)*(7-sel.RecSpeed);			//转矩保护上限设置
			}else
			{
				motor_settings.upper_threshold = AngleCW_T[sel.AngleCW]*d_Torque[sel.Torque]/10;											//转矩保护上限设置
			}
			motor_settings.lower_threshold = AngleCW_T[sel.AngleCW]*d_Torque[sel.Torque]/20;												//转矩取消保护下线设置
		}else
		{
			if(sel.AngleCW < 16)
			{
				motor_settings.upper_threshold = AngleCW_T[sel.AngleCCW]*d_Torque[sel.Torque]/10 + (16 - sel.AngleCW)*(7-sel.RecSpeed);		//转矩保护上限设置
			}else
			{
				motor_settings.upper_threshold = AngleCW_T[sel.AngleCCW]*d_Torque[sel.Torque]/10;	
			}			
			motor_settings.lower_threshold = AngleCW_T[sel.AngleCCW]*d_Torque[sel.Torque]/20;												//转矩取消保护下线设置
		}
		update_settings(&motor_settings);
		start();
	}
...
WorkMode_e ATC_RUN(WorkMode_e mode)
{
	...
	if((flag_running == 0) && (LVGL_TL_Flag == 1))
	{
		...
		if(sel.RecSpeed == 0)
		{
			motor_settings.forward_speed = -(r_Speed[sel.RecSpeed]-50);																	//正转速度
			motor_settings.reverse_speed = (r_Speed[sel.RecSpeed]-50);																	//反转速度
		}else
		{
		...
		motor_settings.upper_threshold = Torque_limit - NoLoadCurrent[r_d_speed[sel.RecSpeed]] - NoLoadCurrent[r_d_speed[sel.RecSpeed]+32];			//转矩保护上限设置
	...
	if((Motor_Error_flag == 1)&&(flag_running == 1))														//检测到MP6570由于故障不处于工作状态
	{
		...
		flag_running = 0;
		Motor_Error_flag = 0;
		Disp_torque_bar_scale1(0,d_TqBlinkGrid[sel.AtrTorque],Disp_TL_limit_flag);
	}else
	{	
		CalculateApex();	
	}
	...
	if((Iq_cnt >= Iq_num)&&(Torque_Display_cnt < 50))
	{
		Torque_Display_cnt ++;
		iq_motor = iq_sum / Iq_cnt;
		Iq_cnt = 0;
		...
		Disp_Torque = (int)(((float)iq_motor)/Torque_p);
		Read_ig_flag = 1;
	}else if((Iq_cnt >= 8*Iq_num)&&(Torque_Display_cnt >= 50))
	{
		Torque_Display_cnt = 51;
		iq_motor = iq_sum / Iq_cnt;
		Iq_cnt = 0;
	...
	if(reciprocate_sw == 2)		
	{
		...
		motor_settings.forward_position = d_AngleCCW[sel.AngleCW] - 28;																						//向前转动角度  前转为负，反转为正
		motor_settings.reverse_position = d_AngleCW[sel.AngleCCW] + 28;																						//向后转动角度
		
		if( -motor_settings.forward_position >= motor_settings.reverse_position )
		{	
			if(sel.AngleCW < 16)
			{
				motor_settings.upper_threshold = (AngleCW_T[sel.AngleCW]*d_Torque[sel.Torque]/10 + (16 - sel.AngleCW)*sel.RecSpeed)*4/3- NoLoadCurrent[r_d_speed[sel.RecSpeed]+32]- NoLoadCurrent[r_d_speed[sel.RecSpeed]];			//转矩保护上限设置
			}else
			{
				motor_settings.upper_threshold = (AngleCW_T[sel.AngleCW]*d_Torque[sel.Torque]/10)*4/3- NoLoadCurrent[r_d_speed[sel.RecSpeed]+32]- NoLoadCurrent[r_d_speed[sel.RecSpeed]];			//转矩保护上限设置
			}
			motor_settings.lower_threshold = motor_settings.upper_threshold*15/16  - NoLoadCurrent[r_d_speed[sel.RecSpeed]+32]- NoLoadCurrent[r_d_speed[sel.RecSpeed]];			//转矩取消保护下线设置
		}else
		{
			if(sel.AngleCCW < 16)
			{
				motor_settings.upper_threshold = (AngleCW_T[sel.AngleCCW]*d_Torque[sel.Torque]/10 + (16 - sel.AngleCCW)*sel.RecSpeed)*3/4;	//转矩保护上限设置
			}else
			{
				motor_settings.upper_threshold = (AngleCW_T[sel.AngleCCW]*d_Torque[sel.Torque]/10)*3/4;	
			}			
			motor_settings.lower_threshold = motor_settings.upper_threshold*7/8;		//转矩取消保护下线设置
		}
		update_settings(&motor_settings);
		start();   
		reciprocate_sw = 0;
...
//	DispApexBar(20-sys.apex);					//显示根测结果	
	if((Disp_TL_limit_cnt > 100)&&(Measure_Flag == 0))
	{
		Apex_RUN(18 - sys.apex);
		Disp_TL_limit_cnt = 0;
		Apex_Ring_Cnt = sys.apex + sel.FlashBarPosition;
		if(Apex_Ring_Cnt > 20)
		{
			Apex_Ring_Cnt = 20;
		}
			//响铃设置
		switch(Apex_Ring_Cnt)
			{
			case 10: {
				ApexRing(40,1);
				break;
			...
			if(Apex_Ring_Cnt < 10) ApexRing(0,0);
...
//	Apex_RUN(20-sys.apex);
	...
	if((RoDebug_pw!=0)&&(mem.MemoryNum == 0))
    {
        psw[Num] = RoDebug_pw;
        Num++;     
		RoDebug_pw = 0;		
...
WorkMode_e RunWithApex(WorkMode_e mode)
{
	...
	//启动初始化
	if((flag_running == 0) && (LVGL_TL_Flag == 2))
	{
		iq_sum = 0;
		Iq_cnt = 0;	
...
//	ResLast_x();
	...
	{
		if(Battery_low_flag != 1)					//低电量，不进行自动启动
		{
	...
	CheckLowBattery();								//待机时检测是否低压，低压不能启动电机
...
//		}
	if(ChargDisp_cnt > 10)
	{
		...
		else
		{
			...
			{
				if(sys.BatteryLevel >= 0)
				{
					charge_bar_cover_len = 13 * (sys.BatteryLevel-1);
				}else
				{
					charge_bar_cover_len = 0;
				}
			}
		}
...
//	flag_bl_off = 1;
	...
	//stop motor
	// beep
	for(i = 0; i < d_backlight[set.backlight]/(set.backlight+1)/2; i++)
	{
		TIM2->CCR1 = TIM2->CCR1 - (set.backlight+1)*2;
		delay_ms(1);
	}
	LCD_Clear(BLACK);													//清屏
...
WorkMode_e MonitorApexStart(WorkMode_e mode)
{
    ...
    if(sel.AutoStart==1 && KeyFuncIndex==0)					//根测自动启动条件，：设置中开启， 当前处于待机界面
    {
			...
			&& 	(sys.ActiveShutRunning == 0))				//主动关机状态清除
        {
            ...
            Ring.short_ring = 1;
			Jump2Menu(1);
			Disp_TL_limit_cnt = 60;				//此处大于100，时为了边扩边测，进入就执行根测显示
//			NextOperate(1);
        }
...
void MonitorApexAction(WorkMode_e mode)
{
	...
	if(flag_running == 1)
	{
		...
		case 1: {														//slowdown
					if(sys.apex>=(18-sel.FlashBarPosition) && ap_flag==0)		//在根尖位置设置值的前一个位置，减速
					{
						ap_flag=1;
...
void Motor2OUTREC()
{
	...
	motor_settings.upper_threshold = Torque_limit - NoLoadCurrent[r_d_speed[sel.RecSpeed]] - NoLoadCurrent[r_d_speed[sel.RecSpeed]+32];			//转矩保护上限设置
	motor_settings.lower_threshold = (Torque_limit - NoLoadCurrent[r_d_speed[sel.RecSpeed]])/2 - NoLoadCurrent[r_d_speed[sel.RecSpeed]+32];		//转矩取消保护下线设置		
