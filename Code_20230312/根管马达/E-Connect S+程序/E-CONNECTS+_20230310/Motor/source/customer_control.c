...
uint16_t Round_Last = 0;
uint16_t Round_Cnt;	
uint16_t Round_CZ = 0;														//两次转动角度差值
uint16_t REV_cnt;
uint32_t REV_jishu = 0;
uint8_t Motor_Error_flag = 0;							//没有错误，为0
extern uint8_t  ap_Action_flag;							//在速度模式作用， = 0，未到根测点   = 1，到达根测点
extern uint8_t Aging_Flag;													
extern uint8_t Aging_Flag1;
extern uint8_t 	flag_running;
extern uint8_t 	ap_flag;
extern uint8_t speed_cnt;
uint8_t iq_shuaxin_flag = 0;
uint8_t motor_qiehuan_flag = 0;
extern uint16_t  Disp_cnt;
extern uint16_t    iq_WUXIAO_cnt;
extern uint16_t Torque_Display_cnt;
void customer_control()
{
	...
	if(motor_settings.mode == 0)						//速度模式
	{
		if(motor_settings.forward_speed < 0)
		{
			if(motor_status.status == 2)
			{
				Round_CZ =  Round_Last-regi[0x60];
			}else
			{
				Round_CZ =  regi[0x60] - Round_Last;
			}
		}else
		{
			if(motor_status.status == 2)
	...
	}else
	{
		if((Aging_Flag == 0)&&(Aging_Flag1 == 0))
		{
			if(F_SetMode == 2)																//校准状态
			{
				if((Round_Cnt >= 5000)&&(motor_settings.mode == 0))
				{
//					Round_Cnt = 5000;
//					if(speed_cnt > 31)
//					{
//						if(Round_CZ > d_Speed[speed_cnt-32] + 100)
//						{
//							Motor_Error_flag = 1;
//						}
//					}else
//					{
//						if(Round_CZ > d_Speed[speed_cnt] + 100)
//						{
//							Motor_Error_flag = 1;
//						}
//					}
			...
			{
				if((Round_Cnt >= 5000)&&(flag_running == 1)&&(motor_settings.mode == 0))		//正常运行状态
				{
					Round_Cnt = 5000;
					if(sel.OperaMode == 3)
					{
						if(Round_CZ > r_Speed[sel.RecSpeed] + 100)
						{
							Motor_Error_flag = 1;
							stop();
						}
					}else if((sel.OperaMode == 0)&&(sel.OperaMode == 1))
					{
						if(Round_CZ > d_Speed[sel.Speed] + 50)
						{
							Motor_Error_flag = 1;
							stop();
						}
...
/******************************************************************/	
	...
	else if((motor_status.mode == 0) &&(motor_status.status != 0))							//速度模式
	{
		...
		{
			if((sys.apex>=(18-sel.FlashBarPosition))&&(sel.ApicalMode == 0))				//根测有反转模式下
			{
				ap_flag = 1;
				sys.over_apex_pos_flag = 1;
				ap_Action_flag = 1;
				motor_status.status = 3;
			}else
			{
...
//				stop();
			if((motor_settings.autorev_mode == 0 || motor_settings.autorev_mode == 1)&& (motor_status.reach_torque == 1 || ap_Action_flag == 1)) //到达扭矩反转 & 扭矩到达
			{
				motor_status.status = 3;  													//反转
				Round_Cnt = 0;
				REV_cnt = 0;
				if(sel.Speed < 7)															// 小于120转
				{
					REV_jishu = 800000/(d_Speed[sel.Speed]);								//回退1圈
				}else
				{
					REV_jishu = 1500000/(d_Speed[sel.Speed]);								//回退2.5圈
				}
				motor_qiehuan_flag = 1;
			}
		}
		else if(motor_status.status == 3)
		{
			...
			update_command = 1;
			if((motor_settings.autorev_mode == 0 || motor_settings.autorev_mode == 1) && (motor_status.reach_torque == 2)&& (ap_Action_flag == 0)&&(REV_cnt > REV_jishu)&&(ap_flag == 0))
			{
				motor_status.status = 2;
				Round_Cnt = 0;
				motor_qiehuan_flag = 1;
...
{
	threshold_times_H = sel.RecSpeed+1;								//超限次数与往复速度正相关
...
void init_registers()
{
	...
	regi[0x27] = 0x07FF;  		//Speed loop maximum limit
