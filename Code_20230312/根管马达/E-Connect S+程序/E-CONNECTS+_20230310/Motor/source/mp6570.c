...
void mp6570_init(unsigned char slave_addr)
{
	...
	//Íù¸´Ð±ÂÊ
//	MP6570_WriteRegister(slave_addr, 0x06, 0x7FF); 						//SH_CMP
...
u8 MP6570_AutoThetaBias(unsigned char slave_addr, u16 Ihold_mA , u16 Thold_ms)//current(mA)?interval(ms)
{
	...
	u16 i54 = MP6570_ReadRegister(slave_addr, 0x54);			//read register 0x54 to determine if MP6570 is at ON or OFF state
	u16 i33 = MP6570_ReadRegister(slave_addr, 0x33);			//read register 0x33 to determine standby or not-standby mode
	if (i54 != 0x00)
	{
	...
	u16 i05 = MP6570_ReadRegister(slave_addr, 0x05);			//read register 0x05
	reg[0x05] = i05 & 0x7fff; 
	...
	u16 THETA_BIAS_TIME=(u16)(Thold_ms*0.1);	     			// calculate THETA_BIAS_TIME value
