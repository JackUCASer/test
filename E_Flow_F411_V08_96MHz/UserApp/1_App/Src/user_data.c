...
//	return tmy_read_user_data("sys_para_t", &sys_para_t, sizeof(sys_para_t));
	static uint8_t databuf[15] = {0};
	REV_U32_T rev_u32_t;
	
	taskENTER_CRITICAL();
	EEPROM_Read(0x20, databuf,15);
	taskEXIT_CRITICAL();
...
void PWR_On_Off_Detect2(void)
{
	...
	if(already_pwr_on_flag == false){				//	A. 关机状态-->上电开机状态
		if(isPWR_OFF() == 0){		//	软件使能开机
			PWR_ON();
			already_pwr_on_flag = true;
		}else if(charge_state == 0){				//	B. 合适的适配器接入
			PWR_ON();
			already_pwr_on_flag = true;
		}
	}else{											//	C. 运行状态下
		if((isPWR_OFF() == 0)&&(charge_state != 0)){
			osDelay(2000);
			if(isPWR_OFF() == 0){	//	2s之后开关机按键仍然被按下，软件使能关机
				Set_VGUS_Page(PAGE_PowerOff);
				osDelay(1000);
				PWR_OFF();
			}	
