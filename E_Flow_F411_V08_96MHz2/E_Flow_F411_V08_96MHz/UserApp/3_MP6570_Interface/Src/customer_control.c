...
// add new function 2022-08-24
void monitor_out_of_control(void);
// end add 
/*****************************set working mode********************************************
...
/*	滑动平均滤波器的缓存	*/
#define IQ_BUF_LEN				5
#define OVER_IQ_TIME			100
#define OVER_IQ_TIME_CALIB		60
...
void customer_control()
{
	...
	static uint8_t wait_err_state_cls = 0;	//	等待error_status清零
	static uint16_t over_iq_cnt = 0;
	...
	movemean_iq = f_iq;
	if((!error_status)&&(!wait_err_state_cls)){	// 当前电机无错误且并未等待错误解除
		switch(motor_run_direction){
			/*-------------------------------- Calibration校正 --------------------------------------*/
			case MOTOR_CALIB_FWD:
				if(f_iq > 120){
					if(++over_iq_cnt > OVER_IQ_TIME_CALIB){
						stop();
			...
			case MOTOR_CALIB_REV:	
				if((f_iq < -60)&&(dynamic_pressure_kPa < 400)){
					if(++over_iq_cnt > OVER_IQ_TIME_CALIB){
						stop();
			...
			case MOTOR_AGING_FWD:
				...
				if((((current_pos&0x8000)==(top_pos&0x8000))&&(current_pos >= top_pos))||(f_iq > 150)){
					stop(); 
			...
			case MOTOR_FWD:
				...
				current_pos = (Get_Current_Handle_Injector_Pos() >> 16);
				top_pos = (handle_injector_t.TOP_LOC >> 16);
				if(((current_pos&0x8000)==(top_pos&0x8000))&&(current_pos >= top_pos)){
					stop(); 
					motor_run_direction = MOTOR_TOP;
				}else if(f_iq > 210){				//	过载，电机停止
					stop();
				}
			break;
			...
			case MOTOR_REV:
				handle_in_origin = false;
				if((f_iq < -60)&&(dynamic_pressure_kPa_F < 400)){	//	保证液压在400kPa以下
					if(++over_iq_cnt > OVER_IQ_TIME){
						stop(); 
			...
			case MOTOR_REV_TO_ORIGIN:
				handle_in_origin = false;
				current_pos = (Get_Current_Handle_Injector_Pos() >> 16);
				origin_pos = (handle_injector_t.ORIGIN_LOC >> 16);
		...
		wait_err_state_cls = 1;	//	等待电机错误解除
		if(!error_status){		//	电机错误解除
			wait_err_state_cls = 0;
			// 启动现场恢复
//			set_speed_mode_speed(recover_speed, recovery_motor_run_dir);
		}else
			return ;
	}
...
//	add 2022-08-24： 电机失控（失速）异常监测
void monitor_out_of_control(void)
{
	static uint16_t last_angle = 0;
	static uint16_t delta_angle_err_cnt = 0;
	// 获取相邻采样角度差 delta_angle
	delta_angle = ((motor_angle - last_angle) <= (last_angle - motor_angle) ? (motor_angle - last_angle) : (last_angle - motor_angle));
	delta_angle = (delta_angle <= (0xFFFF - delta_angle) ? delta_angle : (0xFFFF - delta_angle));
	last_angle = motor_angle;
	delta_angle_cnt ++;
	/*--------------------实验总结-------------------
		1. 电机在当前程序最快转速下： delta_angle ~= 950  <200us间隔下的角度差>
	*/
	if(delta_angle > 1500){ //	如果相邻角度差超过 1.5*max_delta_angle，认为电机转速异常:跑飞了
		if(++delta_angle_err_cnt > 1000){	// 连续200us*1000 = 200ms转速异常，电机停止&PID环路重置
			stop();
			reset_ref_and_loop();
			delta_angle_err_cnt = 0;
			xSemaphoreGiveFromISR(sMotor_goBottom, NULL);	//	启动电机回退程序
		}
	}else
		delta_angle_err_cnt = 0;
}
...
void init_registers()
{
	...
	regi[0x34] = 0x0130; 			//Control Mode[0:1](00:speed, 01:position, 02:torque) reference Mode[2:3](00:digital, 10:Pulse, 11: PWM)
