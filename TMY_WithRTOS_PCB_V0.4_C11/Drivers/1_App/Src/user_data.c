...
const uint32_t injector_speed_t[4][3] = {
	{200*F_DIV,	200*F_DIV,	400*F_DIV},				/*	Normal速度[LOW, LOW, MID]	*/
	{200*F_DIV,	400*F_DIV,	800*F_DIV},				/*	Turbo 速度[LOW, MID, HIGH]	*/
	{200*F_DIV,	200*F_DIV,	200*F_DIV},				/*	STA   速度[LOW, LOW, LOW]	*/
	{400*F_DIV,	800*F_DIV,	800*F_DIV},				/*	回吸；回退1； 回退2	*/
	};
...
{
	/*	now_dis = current_pos - bootom_pos*/
	uint16_t now_dis = 0;
	uint16_t bottom_pos = 0;
	uint32_t remain_dose = 0;
	uint32_t all_dis = (handle_injector_t.DISTANCE >> 16);
	bottom_pos = (handle_injector_t.BOTTOM_LOC >> 16);
	
	if((current_pos&0x8000) == (bottom_pos&0x8000))					/*	如果最高位相同	*/
		if(current_pos <= bottom_pos)
			now_dis = 0;
		else
			now_dis = current_pos - bottom_pos;
	else if( (!(current_pos&0x8000)) && (bottom_pos&0x8000) )		/*	current_pos最高位为0， bottom_pos最高位为1	*/
		now_dis = 65535- bottom_pos + 1 + current_pos;
	else															/*	另外一种情况与实际理论不合，直接置零	*/
		now_dis = 0;
	
	/*	compute remain dose */
	remain_dose = (all_dis - now_dis)*100 / all_dis;
	return remain_dose;
}
...
*********************************************************************************************************
*/
static int8_t tmy_database_init(void)
{
	fdb_err_t result;
...
void power_on_off_detect(void)
{
	...
	static bool database_is_init = false;
...
//			xSemaphoreGive(sMotor_goBottom);			/*	使电机回退至底部	*/
...
//				osDelay(50);
			...
			if(!i8_result)	myPrintf("\r\nSave data success!\r\n");
			else			myPrintf("\r\nSave data failed!\r\n");
			
			PWR_OFF();
		break;
		
		case OFF2_LOW_CHG:	/*	关机键有效，但无法关机	*/
			FDB_PRINT("only show power off but the charger's volatge is too low !\r\n");
			PWR_ON();												/*	开启电池供电	*/
		break;
		
		case OFF3_HIGH_CHG:	/*	关机键有效，但无法关机	*/
			FDB_PRINT("only show power off but the charger's voltage is too high !\r\n");
			PWR_ON();												/*	开启电池供电	*/
		break;
			
		case ON1_CHG:		/*	开机	*/
			PWR_ON();												/*	开启电池供电	*/
			if(database_is_init == false){
				tmy_database_init();								/*	初始化数据库	*/
		...
		case ON3_LOW_CHG:	/*	开机	*/
			...
			FDB_PRINT("notice user that the charger's voltage is too low !\r\n");
		break;
			
		case ON4_HIGH_CHG:	/*	开机	*/
			PWR_ON();												/*	开启电池供电	*/
			if(database_is_init == false){
				tmy_database_init();								/*	初始化数据库	*/
				database_is_init = true;
			}
			FDB_PRINT("notice user that the charger's voltage is too high !\r\n");
		break;
	}
