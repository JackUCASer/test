		...
		switch(lcd_touch_btm->touch_id){
			case PAGE_MODE_FUNC_ID:
				switch(lcd_touch_btm->touch_state){
					case 0x01:
						return MODE_NORMAL;				/*	如果Normal按钮为按下状态	*/
					case 0x02:
						return MODE_FAST;				/*	如果FAST按钮为按下状态	*/
					case 0x04:
						return MODE_PDL;				/*	如果PDL按钮为按下状态	*/
					case 0x08:
						return MODE_SETTING;			/*	如果Setting按钮被按下	*/
					default: break;
				}
				return MODE_IDLE;
...
{
	/*	1. 位置获取	*/
	mode_m_change_dir_t = Mode_Position_Compare(motor_dir_t);
