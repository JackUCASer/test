...
SFLASH_T g_tSF;
extern SPI_HandleTypeDef hspi2;			//	SPI声明
...
//static uint8_t s_spiBuf[4*1024];	/* 用于写函数，先读出整个page，修改缓冲区后，再整个page回写 1*/
...
void bsp_InitSFlash(void)
{
	...
	SF_CS_0();									/* 软件方式，使能串行Flash片选 */
	bsp_spiWrite1(CMD_DISWR);		/* 发送禁止写入的命令,即使能软件写保护 */
	SF_CS_1();									/* 软件方式，禁能串行Flash片选 */
...
uint32_t sf_ReadID(void)
{
	...
	id1 = bsp_spiRead1();					/* 读ID的第1个字节: MF7-MF0 */
	id2 = bsp_spiRead1();					/* 读ID的第2个字节:	ID15-ID8 */
	id3 = bsp_spiRead1();					/* 读ID的第3个字节: ID7-ID0 */
	SF_CS_1();										/* 禁能片选 */
