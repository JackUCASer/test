...
void motor_controller(void)
{
	...
	if(xSemaphoreTake(sMotor_goOrigin, (TickType_t)0) == pdTRUE){		//	回到液压原点
		...
		disable_vgus_touch();	/*	关闭触摸	*/
		set_speed_mode_speed(-REV_SPD, MOTOR_REV);
		while( !check_if_motor_reach_bottom() ){	//	等待电机到达底部
			osDelay(50);
		}
		motor_to_goal_cycle = motor_cycle + DOWN_CYCLE;
...
static bool check_if_motor_reach_origin(void)
{
	CONTROLLER_PRINTF("current_cycle = %d, bootom_cycle = %d, motor_cycle = %d, motor_to_goal_cycle = %d!\r\n",\
				(get_current_handle_injector() >>16), (handle_injector_t.BOTTOM_LOC >>16), motor_cycle, motor_to_goal_cycle);
...
static bool check_if_motor_reach_bottom(void)
{
	CONTROLLER_PRINTF("current_cycle = %d, bootom_cycle = %d, motor_cycle = %d, motor_to_goal_cycle = %d!\r\n",\
				(get_current_handle_injector() >>16), (handle_injector_t.BOTTOM_LOC >>16), motor_cycle, motor_to_goal_cycle);
	if(motor_run_direction == MOTOR_BOTTOM)
		return true;
	else	
		return false;
}
...
static bool check_if_motor_reach_top(void)
{
	CONTROLLER_PRINTF("current_cycle = %d, top_cycle = %d, motor_to_goal_cycle = %d!\r\n", \
	(get_current_handle_injector() >>16), (handle_injector_t.TOP_LOC >>16), motor_to_goal_cycle);
