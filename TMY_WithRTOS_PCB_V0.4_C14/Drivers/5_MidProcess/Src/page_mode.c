...
static void run_dosage_mode(void)
{
	/*	1. 位置获取	*/
	mode_m_change_dir_t = Mode_Position_Compare(motor_run_direction);
	if(mode_m_change_dir_t == MODE_M_FWD_GOAL){
		fix_dose = 0;
		first_run_idle = true;
		run_mode = run_idle_mode;
		return ;
...
//		show_variable_icon(PAGE_Mode, PAGE_MODE_SPEED_ID, 0);		/*	速度清零	*/
//		if(DOSAGE_FLAG == true)
//			show_variable_icon(PAGE_Mode, PAGE_MODE_FUNC_ID, 0x08);		/*	失能所有功能	*/
//		else
//			show_variable_icon(PAGE_Mode, PAGE_MODE_FUNC_ID, 0x00);		/*	失能所有功能	*/
		if(DOSAGE_FLAG == true)
...
static void motor_speed_adjust_accordingto_fp(PAGE_MODE_SELECT page_mode_select)
{
	...
	switch(page_mode_select)
	{
		...
		case MODE_FAST:
		case MODE_DOSAGE:	index = 1;	break;
	...
	if(set_speed_t[index].now_speed != set_speed_t[index].last_speed){
		set_speed_mode_speed(set_speed_t[index].now_speed, MOTOR_FWD);
		if(page_mode_select == MODE_DOSAGE)
