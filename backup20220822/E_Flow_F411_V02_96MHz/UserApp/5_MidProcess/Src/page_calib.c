...
*********************************************************************************************************
*/
#define FWD_SPD				injector_speed_t[3][2]	/*	校正过程中，电机正转速度FWD_SPD和反转速度REV_SPD	*/
...
static OIL_PRESS_CALIB_STATE_T* oil_Pressure_Calibration(void)
{
	...
	{
		case CALIB_READY:{
			set_speed_mode_speed(100, MOTOR_FWD);
			osDelay(200);
			set_speed_mode_speed(FWD_SPD, MOTOR_CALIB_FWD);				/*	开始前进	*/
		...
		case CALIB_FWD_1st:{
			...
			if(motor_reach_endpoint_t == REACH_TOP){
				...
				set_speed_mode_speed(-100, MOTOR_REV);
				osDelay(200);
				set_speed_mode_speed(-REV_SPD, MOTOR_CALIB_REV);			/*	开始回退	*/
		...
		case CALIB_REV_2nd:{
			...
			if(motor_reach_endpoint_t == REACH_BOTTOM){
				...
				osDelay(300);
				motor_to_origin_cycle = motor_cycle + DOWN_CYCLE;		/*	计算电机原点位置	*/
				set_speed_mode_speed(FWD_SPD,MOTOR_CALIB_TO_ORIGIN);							/*	开始前进DOWN_CYCLE圈	*/
