...
void customer_control()
{
	...
	switch(motor_run_direction){
		...
		case MOTOR_REV:
			...
			f_iq = movmean_filter2(iq, f_iq_buf, IQ_BUF_LEN);
			handle_in_origin = pressure_origin_search(dynamic_pressure_kPa_F, get_current_handle_run_distance(current_pos));	// 液压原点寻找
			if((((current_pos&0x8000)==(bottom_pos&0x8000))&&(current_pos <= bottom_pos))||(f_iq < -100)||(handle_in_origin == true)){
				stop();
		...
		case MOTOR_FWD_TO_GOAL:
			...
			if((current_pos&0x8000) == (motor_to_goal_cycle&0x8000)){
				if(current_pos >= motor_to_goal_cycle){
					stop();
					motor_run_direction = MOTOR_FWD_GOAL;
				}	
			}
			break;
